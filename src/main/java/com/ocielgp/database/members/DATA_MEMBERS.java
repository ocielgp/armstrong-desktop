...
public class DATA_MEMBERS {
    ...
    public static QueryRows ReadMembers(int limit, int page, String query) {
        Connection con = DataServer.getConnection();
        ...
        try {
            // Query initial
            // TODO: OPTIMIZE
            String sqlQuery = "SELECT M.idMember, M.name, M.lastName, PM.endDate, M.access FROM MEMBERS M LEFT JOIN PAYMENTS_MEMBERSHIPS PM ON PM.idMember = M.idMember WHERE (M.flag = 1) AND M.idMember NOT IN (SELECT idMember FROM STAFF_MEMBERS WHERE flag = 1) AND (PM.idPaymentMembership = (SELECT DISTINCT idPaymentMembership FROM PAYMENTS_MEMBERSHIPS WHERE idMember = M.idMember ORDER BY startDate DESC LIMIT 1) OR PM.idPaymentMembership IS NULL)";
            ...
            if (filterOnlyActiveMembers) {
                sqlQuery += "AND PM.endDate >= CURDATE() ";
            }
            if (filterOnlyDebtors) {
                sqlQuery += "AND M.idMember IN (SELECT DISTINCT idMember FROM DEBTS WHERE (flag = 1) AND debtStatus = 1)";
            }
